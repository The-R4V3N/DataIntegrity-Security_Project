    ...
    else if (length == AES_BLOCK_SIZE)
    {
        if (sessionId != 0)
        {
            ...
            if (0 == mbedtls_aes_crypt_cbc(&aesEncryptionContext, MBEDTLS_AES_DECRYPT, length, decryptionInitializationVector, buffer, encryptedData))
            {
                ...
                {
                    if (encryptedData[0] == 0x01) // Get temperature
                    {
                        if (0 == memcmp(&sessionId, &encryptedData[1], sizeof(sessionId)))
                        {
                            buffer[0] = 0x10; // OKAY
                            float temp = temperatureRead();
                            sprintf((char *)&buffer[1], "%2.2f", temp); // Get temperature and format it
                        }
                    }
                    else if (encryptedData[0] == 0x02) // Toggle LED
                    {
                        if (0 == memcmp(&sessionId, &encryptedData[1], sizeof(sessionId)))
                        {
                            buffer[0] = 0x10; // OKAY
                            ...
                            sprintf((char *)&buffer[1], "LED");           // Respond with some status
                        }
                    }
